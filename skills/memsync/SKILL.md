---
name: memsync
description: "Trigger: /memsync, sincronizar memoria de sesion, guardar memoria y subir la branch. Saves the session summary to Engram, syncs it to .engram/, and commits+pushes only the resulting chunk to the current branch."
license: MIT
metadata:
  author: hjagar
  version: "0.1.0"
...
- The user types `/memsync`.
- The user asks to "guardar memoria de la sesion y subirla", "sincronizar engram con la branch", or an equivalent session-memory sync request.
...
| New chunk synced, other files also modified/untracked | Ask user: include those files or sync memory only (Hard Rule 3) |
...
2. **Resolve & Check Branch:** Detect the current branch and its protection status per `references/memsync-flow.md`. Protected → stop (Hard Rule 1).
3. **Sync Memory:** Run `engram sync` and detect whether it produced a new/changed chunk per `references/memsync-flow.md`. None → stop (Hard Rule 4).
4. **Check Working Tree:** List pending changes outside `.engram/`. Non-empty → ask the user per Hard Rule 3.
5. **Commit & Push:** Stage the resolved file set, commit with the conventional-commit message from `references/memsync-flow.md`, and push to the current branch.

## Output Contract

Return:
- Sync result: performed / skipped (no new chunk) / blocked (protected branch).
...
- `references/memsync-flow.md` — exact `engram sync` diffing logic, branch-protection detection (with fallback), and the commit/push command sequence.
